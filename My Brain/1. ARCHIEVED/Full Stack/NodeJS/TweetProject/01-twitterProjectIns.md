...
npm i @types/node -D -thêm kiểu ts cho thằng nodejs hiểu (giúp nodejs đọc đc file type script)
...
```bash
touch tsconfig.json  //setting đầu ra của typescript
```
...
```bash
touch .eslintignore   //loại bỏ những cái mình ko muốn nó kiểm tra
```
...
```bash
touch .editorconfig       //cài cho máy tính của em
```
...
```json
{
  "watch": ["src"], //kiểm tra file source, giám sát thư mục source của mình
  "ext": ".ts,.js", //trecking các file có ts và js .kiểm tra những file có đuôi này
  "ignore": [], //liệt kê file nào mà bạn k thích theo dõi vào, ko kiểm tra những file bỏ trong đây
  "exec": "npx ts-node ./src/index.ts" //chạy file index, sử dụng ts-node để đọc đc file ./src/index.ts
}
```
...
```bash
touch type.d.ts   //dùng đĩnh nghĩa những kiểu dữ kiệu dùng trong ts ít sự dụng
```
...
file index.ts
vô để xem express routing [https://expressjs.com/en/guide/routing.html](https://expressjs.com/en/guide/routing.html)

```ts
import express from "express";
const app = express();
//Ở đây, app là một đối tượng của Express.js, được tạo ra bằng cách gọi hàm express(). Đối tượng này được sử dụng để định nghĩa các route handlers, xử lý yêu cầu HTTP, và cấu hình ứng dụng web của bạn. Điều này bao gồm việc xác định các tuyến đường (routes), xử lý yêu cầu từ client và trả lại các phản hồi.
const port = 3000;
app.get("/", (req, res) => {
...
//nếu viết 100 cái route ở đây cũng đc nhưng bị rối , thường chia từng cái route ra vd những cái route liên quan user để riêng,route tweeter để riêng hay route refesh token để riêng
app.listen(port, () => {
...
'http://localhost:3000/'
![[My Brain/1. ARCHIEVED/Full Stack/NodeJS/TweetProject/attachments/image.png]]
...
![[My Brain/1. ARCHIEVED/Full Stack/NodeJS/TweetProject/attachments/image-1.png]]
...
![[My Brain/1. ARCHIEVED/Full Stack/NodeJS/TweetProject/attachments/image-2.png]]
...
usersRouter.use((req, res, next) => {
  ...
  next(); //ok mày đc phép đi tiếp , ko có next thì ko đc xuống (server sẽ xử lý hoài ko đc controller=>pending)
}); //localhost:3000/api/ thì nó ko thấy j hết bên postman nhưng bên terminal thì có time vì chạy đc middleware hay server
```
...
usersRouter.use(
  (req, res, next) => {
    console.log("Time: ", Date.now());
    // next()
    res.status(400).send("not allowed"); //server gửi gói hàng,server thì ko từ chối,vẫn log đc time và nhận đc chữ not allowed và ko xuống đc những thằng dưới nữa
    console.log(12345); //đoạn code này vẫn chạy bên terminal, chỉ là server thấy, còn client k nhận đc, muốn ko có thì return ở res....
  },
  (req, res, next) => {
    console.log("Time2: ", Date.now());
    next();
...
![[My Brain/1. ARCHIEVED/Full Stack/NodeJS/TweetProject/attachments/image-3.png]]
![[image-4.png]]
...
Cảm ơn bạn đã nhắc! Dưới đây là bản viết lại **đúng y chang nội dung gốc của bạn**, nhưng được trình bày lại đẹp hơn, rõ ràng hơn — **không thêm, không bớt gì cả**.
...
* trên thực tế người ta thường tách url này thành các biến để dễ dàng tương thích với nhiều database khác nhau, các biến được tách ra và lưu vào file `.env`, nên giờ sẽ tạo file `.env` ở lớp ngoài cùng của dự án và tiến hành
...
![[My Brain/1. ARCHIEVED/Full Stack/NodeJS/TweetProject/attachments/database.png]]
