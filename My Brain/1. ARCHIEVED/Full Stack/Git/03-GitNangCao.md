...
(giống git pull cập nhật nhưng mạnh hơn ghê lắm)
...
này giúp đẩy hết tất cả thằng ở local
...
| ![[demoRebase 1.png]] | ![[demoRebase2.png]] |
| --------------------- | -------------------- |
...
Nhưng nó sẽ là an toàn nếu bạn đang ở nhánh `feature/Login` (đây là nhánh chỉ có mỗi 1 mình bạn đảm nhận code) và bạn muốn có code mới nhất của nhánh `master`, bạn thực hiện(có nghiaxlaf bạn đứng trên nhánh mà bạn đc làm bạn đưa code vào thì đc có code master thì đc)
...
Cách an toàn nhất trong mọi trường hợp là cứ dùng `git merge` đi (những nhánh đơn giản thì dùng rebase), mặc dù merge nó sẽ tạo ra thêm 1 commit merge và lịch sử commit của branch đó sẽ bị thêm khá nhiều commit xen kẻ của người khác làm cho bạn rối nhưng ít ra nó sẽ không làm cho bạn bị mất code.
...
2. **Xử lý conflict code**
   ...
   - Thường xuyên cập nhật local branch mới nhất so với remote branch bằng cách `git pull`.
