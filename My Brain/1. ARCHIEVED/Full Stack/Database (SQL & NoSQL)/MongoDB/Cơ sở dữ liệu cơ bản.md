...
- Schema(cấu trúc - table) chặt chẽ (schema ra đời để định nghĩa cho object, để mô tả 1 đối tượng của 1 table nào đó, khi đó đối tượng đó "." sẽ đổ ra những thứ liên quan đối tượng đó)
- Khuyến khích các tiêu chuẩn chuẩn hóa để giảm thiểu sự dư thừa dữ liệu
- Có thể mở rộng nhưng sẽ hơi tốn công (tăng thuộc tính phải tạo thêm table hoặc là đập làm lại,ko có chuyện nới) mở rộng theo chiều sâu

## NoSQL

- Dữ liệu được lưu dưới dạng JSON với các cặp key-value
- Không cần Schema(lý thuyết mấy khứa youtube), lưu được hầu như bất cứ thứ gì
- Hiệu năng tuyệt vời, mở rộng theo chiều ngang dễ dàng (có nghĩa là thêm đc thuộc tính) 1 table có chiều dọc và chiều ngang nhưng thêm thì nhớ chỉnh lại schema, nếu ko ai muốn chuyền gì chuyền là đi

  req=>middleware=>(service)=>controller=>(service)=>server sẽ tạo 1 đoạn mã =>lưu user vào server=>gửi cái verify-token cho cái j đấy mà mày cung cấp
![[schema.png]]

## Ví dụ về sự ưu việt của NoSQL (đừng tin 100%)
...
**Vấn đề 1**: Sau khi code được 1 thời gian chúng ta nhận ra rất ít người có 1 số điện thoại, thường họ sẽ có SDT nơi làm việc, ở nhà, cá nhân,... Vậy nên chúng ta không thể thêm `telephone1`, `telephone2`,... vào bảng `contacts` được. Chúng ta sẽ tạo thêm 1 bảng riêng là `telephones` vậy để giải quyết đa trị thì tách ra thêm 1 table riêng ròi liên kết 2 table lại với nhau = cái mã của đối tượng
...
- id
- contact_id //này là khóa ngoại từ cái id contacts
- address
...
Khi chúng ta muốn query để lấy data đầy đủ của một contact, chúng ta phải dùng JOIN kết hợp với hàng chục bảng liên quan ,mỗi lần join hay lookup hiệu năng sẽ giảm xuống nhiều
...
## Khi nào nên dùng MongoDB (biết js là đc)
...
- Khi muốn tích hợp lượng data lớn (lưu data = chuỗi thì lưu đc nhìu lắm)
- Data có cấu trúc phức tạp (đặc trưng mongoDB là ko có cấu trúc schema rõ ràng nên muốn lưu đối tượng có nhiều thuộc tính tào lao là thoải mái luôn)
- Khi cần một CSDL có khả năng mở rộng nhanh, rẻ, còn miễn phí nữa
- Khi cần một CSDL giúp tốc độ phát triển phần mềm nhanh vì mongoDB rất dễ học và rất dễ phát triển nó, biết json là làm đc và thiết kế rấ là nhanh

## Khi nào nên dùng SQL(học sql trước đã)
...
- NoSQL
- Ra đời 2007
- Thay vì lưu trong từng hàng hay cột như SQL, MongoDB dùng document(ở đây là 1 đối tượng 1 data) theo dạng BJSON(ko khác gì json). Ứng dụng có thể lấy data ra theo dạng JSON
...
- Trong mỗi database chúng ta lại có các collection(chính là table)
- Mỗi collection lại có nhiều document vì nó là 1 đống chữ(chính là data 1 object 1 đối tượng)

VPS (Virtual Private Server) là một loại dịch vụ lưu trữ trên máy chủ ảo, và nó là một thành phần quan trọng trong lĩnh vực của máy chủ và hosting web. VPS cung cấp một phần của một máy chủ vật lý với các tài nguyên riêng tư và cách quản lý độc lập. Điều này có nghĩa rằng một máy chủ vật lý có thể được chia thành nhiều máy chủ ảo hoạt động độc lập, mỗi máy chủ ảo có hệ điều hành và tài nguyên riêng.
...
cái mã đc đĩnh dạng cái kiểu là ObjectId
...
- search: có 1 ô type a query
  ...
  - ngoài ra ta còn có more option:
    project: {age: 0} nghĩa là k muốn hiển thị kết quả có age , 1 là hiện đúng cái age
    sort:-1 là giảm dần, 1 tăng dần
    skip: bỏ qua các doc
    limit: giới hạn doc trả về

**bonus {age : {$gt: 19}} : lớn hơn 20 tuổi**
![[My Brain/1. ARCHIEVED/Full Stack/Database (SQL & NoSQL)/MongoDB/attachments/image.png]]
...
- Ứng dụng có tính chất đọc hay ghi nhiều?(lưu trư hay lấy nhiều hơn, hay thêm xóa sủa nhiều)
- Dữ liệu nào thường được truy cập cùng nhau?(chỗ nào truy cập liên tục , chỗ nào lâu mới truy cập)
- Các yếu tố về hiệu suất của bạn như thế nào?
- Dữ liệu của bạn sẽ tăng và mở rộng như thế nào?

## 1 - so sánh SQL và NoSQL

mô hình này gọi là refering:tham chiếu bảng bị phân rã làm 2
![[My Brain/1. ARCHIEVED/Full Stack/Database (SQL & NoSQL)/MongoDB/attachments/image-1.png]]

ở đây ta có thể thấy rằng paul ở hữu rất nhiều tiền tệ, xe
nên cần phải tách ra thành các table như vậy(vì sợ bị multible value bị đa trị,1cootj chứa nhìu giá trị qua khó tìm kiếm), việc này làm ta phải join để có full thông tin

nếu ta dùng nosql thì nó sẽ như thế này (lưu dưới dạng object parse ra đc mảng xài luôn)
Refering và embedding 2 lựa chọn
...
```json
{
  ...
  "profession": ["banking", "finance", "trader"], //này gọi là embedding nhúng
  "cars": [
    {
      "model": "Bentley",
      "year": 1973
...
**lưu ý**: ít thì nhúng nhiều thì tách

## Quan hệ

trong này vẫn có các quan hệ 1-1, 1-few(ít), 1-n(mảng rất nhiều), ít-nhiều, nhiều-nhiều đc tao từ 2 thằng 1-nhiều,ít-ít là 1 ít và ít 1
ta ví dụ với 1 doc
![[image-2.png]]
...
- Field `createAt`(bắt buộc) và `updateAt`(ko có cũng đc) nên có kiểu dữ liệu là `Date` để dể dàng sắp xếp, lọc, tìm
- `createAt` luôn nên có khi tạo document
- `updateAt` thì là 1 optional(tùy chỉnh, muốn hay k cũng đc)
- trường id thì nên là `ObjectID` , \_id thì nên đc tạo = hàm ObjectId(), thằng này giống new Identity trong DBi tự tạo đoạn chuỗi cấm trùng trên toàn bộ
...
```ts
//enum: hộ trợ cho mình viết code rõ nghĩa hơn
...
- Authentication là quá trình xác thực người dùng (nghĩa là xác định mày là ai)
...
- Bước 2: Server sẽ kiểm tra thông tin định danh của client với thông tin trong database. Nếu thông tin định danh đúng, server sẽ trả về một dấu hiệu(sign) gì đó để cho client biết là đăng nhập thành công.
...
- Bước 4: Server sẽ kiểm tra dấu hiệu(sign), nếu hợp lệ, server sẽ trả về tài nguyên cần thiết.
  **dấu hiệu ở đây chính là token-sign**
  mô tả:
- khi ta muốn đăng nhập ta đua username/password cho server
- server kiểm tra username/password có đúng hay ko , nếu đúng thì sẽ tạo ra 1 token-sign
  và trả về cho client
  - 1 token-sign sẽ gồm:
    - header:lưu thông tin mã hóa
    - payload:dữ liệu đc lưu trong token, thường là thông tin của user
    - secretkey:chữ ký của server,vd:123!@#
  - sau khi tạo ra token-sign trông như này:
eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c

![[database.png]]
  - server sẽ đưa cho client giữ
- vậy mỗi lần mà client muốn truy cập vào tính năng nào đó, thì họ ko cần gửi username/password nữa, chỉ cần gửi token-sign lên server
- server nhận token-sign và kiểm tra (decode mã hóa ngược lại) xem token-sign(sign) có chữ ký giống secretkey trong token đó có giống secretkey trong database ko
  xác thực(Authentication):
  - nếu giống: server xem payload và biết bạn là ai
  - nếu ko: server biết token-sign này là fake, và sẽ ko cho bạn truy cập vào tính năng đó
    **nên cái token-sign là đại diện cho em luôn, nếu 1 người nào đó ăn cắp đc cái này của em thì người đó sẽ là em**

### 2. Authentication với basic Authentication

- Khi bạn truy cập website sử dụng cơ chế Basic Authentication, server sẽ kiểm tra Authorization trong giao thức HTTP header(truyền dữ liệu qua param,đường dẫn). Nếu Authorization không hợp lệ, server sẽ trả về một response với WWW-Authenticate nằm trong header. Cái này nó sẽ làm website bạn hiển thị popup yêu cầu bạn nhập username/password.
...
Hệ thống sẽ dùng JWT(json web token) để xác thực người dùng. Vậy mỗi lần người dùng đăng nhập thành công thì sẽ tạo ra 1 JWT access token và 1 refresh token.vd acsess token hết hạn trong 30s và refresh token trong 30 ngày thì cứ mỗi 30s thì máy tính sẽ cầm refresh token gửi lên server và server đưa lại 2 cái access và refresh token mới và refresh vẫn cùng ngày hết hạn với cái cũ. sau 30 ngày refresh hết hạn thì sẽ bắt em đăng nhập lại từ đầu để nhận lại 2 cái mới với hiệu lực mới
...
table refresh token trông như này
...
interface Tweet {
  ...
  hashtags: ObjectId[]; //ko lưu mấy cái chuỗi mà anh sẽ có cái table hashtag, vì lưu chuỗi rất khả năng bị lặp nên anh sẽ lưu table hashtag,vì mối quan hệ của table hashtag là ít nhiều nghĩa là 1 bài viết thì có 1 ít hashtag nhưng 1 hashtag thì có ngàn bài viết,nói chung thấy chữ nhiều là tách bảng khỏi nghĩ
  mentions: ObjectId[]; //những người mình tag vào
  medias: Media[]; //lưu ảnh và video
